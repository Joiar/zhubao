...
if($action == 'post')
{
        $r = "/^(13[0-9]{9})|(14[57][0-9]{8})|(15[012356789][0-9]{8})|(17[0-9]{9})|(18[0-9]{9})$/";

    if(!$_POST['name']){
            echo "<script>alert('姓名不能为空！');window.history.go(-1);</script>";exit;
    }elseif(!preg_match($r,$_POST['mobile'])){

            echo "<script>alert('请输入正确的手机号！');window.history.go(-1);</script>";exit;
